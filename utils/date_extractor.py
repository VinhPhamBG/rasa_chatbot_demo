...
REGEX_DATE = (
    r"(3[01]|[12][0-9]|0?[1-9])[-\/:.|](1[0-2]|0?[1-9])([-\/:|.](2[0-1][0-9][0-9]))"
)
...
def regex_date(msg, timezone="Asia/Ho_Chi_Minh"):
    ...
    pattern = re.compile(
        "(%s|%s|%s)" % (regex, regex_month_year, regex_day_month), re.UNICODE
    )
    matches = pattern.finditer(msg)
    regex_pos = []
    for match in matches:
        _dt = match.group(0)
        regex_pos.append(match.span())
        _dt = (
            _dt.replace("/", "-")
            .replace("|", "-")
            .replace(":", "-")
...
def preprocess_msg(msg):
    ...
    for i in range(len(list_word)):
        if (
            i > 0
            and list_word[i].isnumeric()
            and list_word[i - 1] not in ["ngày", "tháng", "năm", "thứ"]
        ):
...
def tokenize(msg):
    ...
    with open("utils/date_synonyms.json", encoding="utf-8") as jsonFile:
            ...
            if data[token] == "day":
                if words[i].isnumeric():
                    ...
                    if (temp + " " + words[i + 2]) in number_str.keys():
                        tokens.append(
                            {data[token]: str(number_str[temp + " " + words[i + 2]])}
                        )
                        remove_token(words, token)
                        continue
...
def get_date(tokens, timezone, dates=[]):
    ...
    while i < len(tokens):
        ...
        if len(dates) > 1 and tok_key == "between":
            ...
            days = [
                start_d + datetime.timedelta(days=x)
                for x in range((end_d - start_d).days + 1)
            ]
            for day in days:
                day = day.strftime("%d-%m-%Y")
                if day not in dates:
